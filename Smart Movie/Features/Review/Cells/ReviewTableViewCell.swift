...
//    }
    ...
    func bindData(data: ResponseReviewModel) {
        authorNameLabel.text = data.userName
        contentLabel.text = data.content
        formatRating(data.vote)
        rateLabel.text = "\(data.vote)/10"
        timeWriteLabel.text = data.time
    }
    
    private func formatRating(_ rate: Int) {
        if rate == 0 {
            return
        } else if rate <= 2 {
